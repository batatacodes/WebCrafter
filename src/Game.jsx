...
/*
  ...
  Corte de árvore: agora funciona com base apenas na distância (1-3m).
*/
...
export default function Game(){
  ...
  // UPDATED: chop logic now cuts the nearest tree within distance (1-3m),
  // camera angle no longer matters.
  function handleChop(){
    const btn = document.querySelector('.chop-btn');
    btn?.classList.add('active');
    setTimeout(()=>btn?.classList.remove('active'),160);

    const player = playerRef.current;
    const trees = treesRef.current;
    if(!player || !trees) { setHudText("Aguarde..."); return; }

    // find nearest tree by distance to player
    let nearest = null;
    ...
    for(let i=0;i<trees.length;i++){
      ...
      const dist = tempPos.distanceTo(player.mesh.position);
      if(dist < nearestDist){
        nearestDist = dist;
        nearest = root;
      }
    ...
    if(!nearest){
      setHudText("Nenhuma árvore por perto!");
      return;
    }

    if(nearestDist < 1 || nearestDist > 3){
      setHudText("Fique a 1-3 metros da árvore para cortá-la.");
      return;
    }

    // animate removal of nearest tree
    const parent = nearest.parent;
    const idx = trees.indexOf(nearest);
    if(idx === -1){
      setHudText("Erro ao cortar.");
      return;
    }

    const start = performance.now();
    const duration = 300;
    function animateCut(now){
      const t = Math.min(1, (now - start)/duration);
      const s = 1 - t;
      nearest.scale.setScalar(s);
      if(t < 1) requestAnimationFrame(animateCut);
      else {
        // remove and spawn new
        parent.remove(nearest);
        trees.splice(idx,1);
        // spawn new tree somewhere else (not too close)
        let tries = 0;
        while(tries < 40){
          const angle = Math.random()*Math.PI*2;
          const radius = 6 + Math.random()*(WORLD_RADIUS-6);
          const x = Math.cos(angle)*radius + (Math.random()-0.5)*6;
          const z = Math.sin(angle)*radius + (Math.random()-0.5)*6;
          const distp = Math.hypot(x - player.mesh.position.x, z - player.mesh.position.z);
          if(distp > 4){
            const trunkGeo = new THREE.CylinderGeometry(0.25,0.35,1.6,8);
            const trunkMat = new THREE.MeshStandardMaterial({color:0x6b3f1f});
            const trunk = new THREE.Mesh(trunkGeo, trunkMat);
  ...
  return (
    <div style={{width:'100%', height:'100%'}}>
      ...
      <div className="overlay-ui">
        ...
        <div
          className="chop-btn"
          onTouchStart={(e)=>{ e.preventDefault(); handleChop(); }}
          onMouseDown={(e)=>{ e.preventDefault(); handleChop(); }}
        >
          CORTAR
        </div>
